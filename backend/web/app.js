(() => {
  ...
  function voiceErrorMessage(code) {
    const map = {
      "not-allowed":
        "마이크가 차단되었습니다. 주소창 왼쪽 자물쇠(또는 마이크 아이콘)에서 마이크를 허용한 뒤 다시 🎤를 눌러 주세요.",
      "service-not-allowed":
        "이 사이트에서 마이크가 차단되었습니다. 크롬 설정 → 개인정보 및 보안 → 사이트 설정 → 마이크에서 허용해 주세요.",
      "audio-capture":
  ...
  function bindVoiceInputs() {
    ...
    setVoiceHint(
      "🎤 버튼을 누르고 말한 뒤, 끝나면 다시 눌러 중지하세요. PC는 주소창에서 마이크 허용이 필요합니다."
    );
    let active = null;
    let activeBtn = null;
    ...
    document.querySelectorAll(".mic-btn").forEach((btn) => {
      btn.addEventListener("click", async (ev) => {
        ...
        try {
          ...
          stopActive();
          setVoiceHint(
            "음성 입력을 시작하지 못했습니다. 페이지를 새로고침한 뒤 마이크를 허용해 주세요."
          );
        }
      });
    });
